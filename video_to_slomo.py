...
def extract_frames(video, outDir):
    ...
    try:
        subprocess.check_call(
            '{} -i "{}" -vsync 0 {}/%06d.png'.format(os.path.join(args.ffmpeg_dir, "ffmpeg"), video, outDir))
...
def create_video(dir):
    error = ""
    print('{} -r {} -i {}/%d.png -vcodec ffvhuff {}'.format(os.path.join(args.ffmpeg_dir, "ffmpeg"), args.fps, dir,
                                                            args.output))
    try:
        subprocess.check_call(
            '{} -r {} -i {}/%d.png -vcodec ffvhuff "{}"'.format(os.path.join(args.ffmpeg_dir, "ffmpeg"), args.fps, dir,
                                                                args.output))
    except CalledProcessError:
